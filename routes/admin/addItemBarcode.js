...
const insertInventoryQuary = "insert into inventory(item_id, item_name, quantity, last_updated) values(?, ?, ?, CURRENT_TIMESTAMP)";
...
router.get('/:barcode', async (req, res) => {
    ...
    try {
        ...
        if (result.length > 0) {
            ...
            return res.status(400).json({ failed: "바코드가 존재하지 않습니다." });
        }
    } catch (err) {
...
router.post('/', async(req, res) => {
    ...
    try{
        ...
        if (result.length > 0) {
            const { item_name, item_id } = result[0];
            await executeQueryPromise(insertInventoryQuary, [item_id, item_name, quantity]);
