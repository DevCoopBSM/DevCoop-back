...
router.post("/", async(req, res) => {
  ...
  try {
    const query = "SELECT student_number FROM users WHERE student_name = ?";
    const [result] = await executeQueryPromise(query, [student_name]);

    if(!result) { // if문 조건에 뭐가 더 필요할까?
      return res.status(400).json({ error : "존재하지 않는 사용자입니다." });
    }

    query = "DELETE FROM users WHERE student_name = ?";
    result = await executeQueryPromise(query, [student_name])
