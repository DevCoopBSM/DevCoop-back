    ...
    try {
      for (const item of jsonData) {
        ...
        if (existingData.length > 0) {
          ...
          await executeQueryPromise(updateQuery, [
            PROD_NM,
            BAR_CD,
            SALE_UPRC,
            PROD_CD,
          ...
          await executeQueryPromise(insertQuery, [
            PROD_CD,
            PROD_NM,
            BAR_CD,
            SALE_UPRC,
      ...
      res
        .status(200)
        .json({ message: "데이터가 성공적으로 업데이트 또는 삽입되었습니다." });
    } catch (error) {
      console.error("데이터 업데이트 또는 삽입 오류:", error);
      res
        .status(500)
        .json({ error: "데이터 업데이트 또는 삽입 중 오류가 발생했습니다." });
    }
  }
